...
.PHONY: help build run watch dev-env ensure-secret-key certs tokens rotate-tokens rotate-certs \
...
$(ENV_FILE):
	...
	umask 077; { \
		...
		for name in DBGRAPH_WEB_VIEWER_TOKEN DBGRAPH_WEB_EDITOR_TOKEN \
			...
			DBGRAPH_MCP_AGENT_TOKEN DBGRAPH_MCP_REVIEWER_TOKEN DBGRAPH_MCP_ADMIN_TOKEN \
			DBGRAPH_SECRET_KEY; do \
